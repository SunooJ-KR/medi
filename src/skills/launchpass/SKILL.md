...
2. `templates/readiness_report.html`을 기반으로 채운다. **리포트는 [한국어]/[타겟국가어] 두 탭으로 완전 병행**한다:
   - 헤더(언어 공통, 1회): 범용 제목(런칭 레디니스 리포트) 고정 + 제품명(소제목) · 타겟 시장 · 분석일시 · 신호등 판정 배너 · 데이터 버전·기준일 · (stale 시) freshness 경고. 타겟어 탭 이름은 `{{TARGET_LANG}}`(예: `日本語`/`English`/`Tiếng Việt`).
   - **KO 패널**(`한국어` 토큰): 1~5 섹션을 전부 한국어로. 현지어 카피는 한국어 번역으로 제시.
   - **ORIG 패널**(`O_` 접두 토큰, `o_` 접두 반복 블록): 동일한 1~5 섹션을 타겟국가어로 완역. 페르소나 승인 카피·NG/대체 표현은 **실제 사용할 현지어 원문 그대로**.
   - 섹션 구성(양 패널 공통): ① 요약(판정 근거 3줄+항목 수) ② 시장 스캔(트렌드·경쟁 제품표+concerns 격자 매칭 강조) ③ 규제 검증★(원본 카피 위반 하이라이트+룰 표+통과율) ④ 페르소나 카드(N개)+PASS 배지+파생 근거·채널 ⑤ 액션(필수/권장 pill, 이모지 없음)+참고 자료.
   - 두 패널의 판정·수치(통과율·건수)는 동일 근거를 각 언어로 표기(내용 불일치 금지).
