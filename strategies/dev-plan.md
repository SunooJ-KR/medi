...
### Phase 5 — 리포트 개선 (제출 후 보강)

**목표**: 리포트 가독성·활용성 개선.

| # | 작업 | 산출물 | 완료 기준 |
|---|------|--------|-----------|
| 5-1 | 리포트 다국어 표기 | `templates/readiness_report.html`, output 샘플 | 리포트를 **한국어 / 타겟국가어 두 탭으로 완전 병행**. 1~5 전 섹션을 양쪽 탭에 각 언어로 완역 표기. 탭 전환은 CSS만(무JS·자립형). 공유 요소(제품명·신호등·버전·통과율 수치)는 헤더에 1회. STAGE 4 절차·샘플(GlowC)에 반영, 태그 균형·플레이스홀더 보존 |

**의존성**: Phase 4(디자인 개편 포함) 완료 후.
